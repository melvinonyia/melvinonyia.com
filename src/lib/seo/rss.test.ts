...
const CHANNEL = {
  ...
  description: 'Essays from example.com',
}
...
describe('buildRss', () => {
  ...
  it('emits one <item> per entry with title, link, guid, pubDate', () => {
    ...
    expect(xml).toContain('<title>First essay</title>')
  ...
  it('omits <description> when no excerpt is provided', () => {
    const xml = buildRss(CHANNEL, [
      { title: 'No excerpt', link: 'https://example.com/x', date: '2025-01-01' },
    ])
    expect(xml).not.toContain('<description>No excerpt</description>')
  })
  ...
  it('formats pubDate as RFC 822-compatible UTC', () => {
    const xml = buildRss(CHANNEL, [
      { title: 'X', link: 'https://example.com/x', date: '2025-11-12' },
    ])
    expect(xml).toMatch(/<pubDate>[A-Z][a-z]{2}, \d{2} [A-Z][a-z]{2} 2025 .*GMT<\/pubDate>/)
  })

  it('lastBuildDate is the date of the most recent entry', () => {
    const xml = buildRss(CHANNEL, [
      { title: 'Newer', link: 'https://example.com/a', date: '2025-11-12' },
      { title: 'Older', link: 'https://example.com/b', date: '2025-08-04' },
    ])
