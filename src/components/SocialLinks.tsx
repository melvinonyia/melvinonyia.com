...
export function SocialLinks({ className, labelClassName }: SocialLinksProps) {
  return (
    <ul className={className ?? 'flex items-center gap-4'}>
      {SOCIAL_LINKS.map((social) => (
        <li key={social.key}>
          <a
            ...
            className="inline-flex items-center gap-2 text-muted transition-colors hover:text-fg focus-visible:text-fg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent"
          >
            <Icon social={social} />
            <span className={labelClassName ?? 'sr-only'}>{social.label}</span>
          </a>
        </li>
      ))}
    </ul>
